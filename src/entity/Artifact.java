...
public class Artifact extends FallObject  implements Interactable,Collidable{	
	...
	public Artifact(double x,int type) {
		super(x, 520, 50, 50);
		// TODO Auto-generated constructor stub
		this.type = type;
	}
	
	public Artifact(double x,double y, int type) {
		super(x, y, 50, 50);
		// TODO Auto-generated constructor stub
		this.type = type;
//		setVy(-5);
	...
	public static void generate(int x) {
		SceneManager.getInstance().getInteractable().add(new Artifact(x, r.nextInt(6)));
		dropSound.play();
	}
	
	public static void generate(double x,double y) {
		SceneManager.getInstance().getInteractable().add(new Artifact(x, y, r.nextInt(6)));
		dropSound.play();
	}
	
	public static void generate(int lowerbound,int upperbound) {
		SceneManager.getInstance().getInteractable().add(new Artifact(r.nextInt(upperbound-lowerbound)+lowerbound, r.nextInt(6)));
		dropSound.play();
	}
	
	public static void generate(int type,int lowerbound,int upperbound) {
		SceneManager.getInstance().getInteractable().add(new Artifact(r.nextInt(upperbound-lowerbound)+lowerbound, type));
		dropSound.play();
	}
